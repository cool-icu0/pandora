...
import {Divider, Flex, message, Card, Calendar, Badge, Statistic, Row, Col, Spin, Tabs, Avatar} from "antd";
...
export default function HomePage() {
  ...
  const user = useSelector((state: RootState) => state.loginUser);
    ...
    const [recommendType, setRecommendType] = useState('similar');
        ...
        <div id="homePage" className="max-width-content">
            <div className="home-layout two-column-layout">
                ...
                <div className="content-column content-right">
                    ...
                    {/* 推荐区域 */}
                    <Card className={`recommend-card rich-card ${!user?.id ? 'blur-card' : ''}`} bordered={false}>
                        {!user?.id && (
                                                    ...
                                                    {userRecommends.map((item: any) => (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="recommend-info">
                                                                <div className="recommend-name">{item.recommendUser.userName}</div>
                                                                <div className="recommend-profile">{item.recommendUser.userProfile}</div>
                                                            </div>
                                                            匹配度 {item.score*100}%
                                                        </Link>
                                                    ))}
