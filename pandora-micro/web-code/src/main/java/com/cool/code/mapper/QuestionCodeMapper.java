...
public interface QuestionCodeMapper extends BaseMapper<QuestionCode> {
    ...
    @Select("SELECT COUNT(DISTINCT q.id) " +
            ...
            "AND s.createTime >= #{date} " +
            "AND s.isDelete = 0")
    Integer countUserCompletedQuestions(@Param("userId") Long userId,
    ...
    @Select("SELECT DISTINCT q.* " +
            ...
            "AND s.isDelete = 0 " +
            "AND q.isDelete = 0 ")
    List<QuestionCode> selectRecentCompletedQuestions(@Param("userId") Long userId,
                                                      @Param("date") String date);
    ...
    @Select("SELECT DISTINCT q.id " +
            ...
            "AND s.isDelete = 0 " +
            "AND q.isDelete = 0")
    List<Long> selectCompletedQuestionIds(@Param("userId") Long userId);
}
